    ...
    *,
    ...
    for index, left_id in enumerate(drone_ids):
        for right_id in drone_ids[index + 1 :]:
            if fovs[left_id] & fovs[right_id]:
                overlap_pairs.append((left_id, right_id))

    if overlap_pairs:
        reward -= 2.0

    return reward, {"fov_overlap": bool(overlap_pairs), "overlap_pairs": overlap_pairs}
...
def build_team_metrics(
    *,
    ...
    total_cells = grid_size * grid_size
    unique_cells_visited = set().union(*(drone.visited_cells for drone in drones.values())) if drones else set()
    return {
        **dict(metrics),
        "coordination_mode": "decentralized_no_communication",
        "reward_mode": "shared_global",
        "team_unique_cells_visited": len(unique_cells_visited),
        "team_coverage_ratio": len(unique_cells_visited) / float(total_cells),
